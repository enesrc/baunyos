...
export default function QuickAccessCard({ item }: { item: QuickAccess }) {
  ...
  const title = translate(item.title_en, item.title_tr);
  const description = translate(item.desc_en, item.desc_tr);
  ...
  return (
    <Link
      ...
      className="group relative overflow-hidden border border-light-3 bg-white p-4 transition-all
        myhover:border-amber myhover:shadow-lg
        dark:border-dark-2 dark:bg-dark-5 dark:myhover:border-amber-bright
    ...
    >
      <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-md
        ...
        group-myhover:bg-amber/20 dark:bg-amber-dull/20 dark:text-amber-bright dark:group-myhover:bg-amber-dull/30">
        <Icon size={26} weight="duotone" />
      </div>

      <div className="min-w-0 flex-1">
        <p className="text-lg font-medium text-dark-2 transition-colors
          group-myhover:text-amber-dull dark:text-light-1 dark:group-myhover:text-amber-bright
          sm:truncate">
          {title}
        </p>
      ...
      <ArrowUpRightIcon
        ...
        className="hidden sm:block mt-1 shrink-0 text-light-4 transition-all
          group-myhover:translate-x-0.5 group-myhover:-translate-y-0.5 group-myhover:text-amber
          dark:text-light-4 dark:group-myhover:text-amber-bright"
      />
    </Link>
  );
}
